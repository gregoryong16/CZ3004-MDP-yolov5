...
def run(weights=ROOT / 'yolov5s.pt',  # model.pt path(s)
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        # NMS
        global CONST_CLASSES
        CONST_CLASSES = classes

        # Apply NMS
        pred = non_max_suppression(pred, conf_thres, iou_thres, classes, agnostic_nms, max_det=max_det)
        dt[2] += time_sync() - t3

        focal_length_sticker = get_focal_length(device, half, model)
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    distance = distance_finder(focal_length_sticker, object_width_in_frame, KNOWN_STICKER_WIDTH);

                    if save_img or save_crop or view_img and conf > conf_thres:  # Add bbox to image
                        ...
                        cv2.imwrite(f"{save_path}\\ - {label.split(',')[0]} - {conf:.2f}.jpg",im0)
                        image_detected(names[c].split(',')[0])
                        d+=1
                        if save_crop:
    ...
    for i, det in enumerate(pred):  # detections per image
        ...
        if len(det):
            ...
            for *xyxy, conf, cls in reversed(det):
                ...
                focal_length_found = focal_length(ref_image_object_width, KNOWN_STICKER_DISTANCE, KNOWN_STICKER_WIDTH)
    return focal_length_found


def get_focal_length(device, half, model):
    bus_img_path = 'images/train/images/20cm0006_jpg.rf.a9e6e5e8f06156cf36a84cde5296b7fc.jpg'
    focal_length_bus = ref_img_information(bus_img_path, device, half, model)
    return focal_length_bus
